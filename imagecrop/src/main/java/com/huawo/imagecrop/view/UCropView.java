...
public class UCropView extends FrameLayout {
    ...
    private void setListenersToViews() {
        mGestureCropImageView.setCropBoundsChangeListener(new CropBoundsChangeListener() {
            @Override
            public void onCropAspectRatioChanged(float cropRatio, float cornerRadiusRatio) {
                mViewOverlay.setTargetAspectRatio(cropRatio, cornerRadiusRatio);
            }
        });
        mViewOverlay.setOverlayViewChangeListener(new OverlayViewChangeListener() {
            @Override
            public void onCropRectUpdated(RectF cropRect, float cornerRadius) {
                mGestureCropImageView.setCropRect(cropRect, cornerRadius);
            }
        });
    }
    ...
    public void resetCropImageView() {
        ...
        setListenersToViews();
        mGestureCropImageView.setCropRect(getOverlayView().getCropViewRect(), 0);
